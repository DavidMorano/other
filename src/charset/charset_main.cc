/* charset_main SUPPORT (charset) */
/* charset=ISO8859-1 */
/* lang=C++20 */

/* Character-Set (setting character-set on files) */
/* version %I% last-modified %G% */

#define	CF_DEBUG	0		/* debugging */

/* revision history:

	= 2000-03-01, David A-D- Morano
	This subroutine was originally written.

*/

/* Copyright © 2000 David A-D- Morano.  All rights reserved. */

/*******************************************************************************

	Names:
	charset

	Description:
	This program sets the character set on the given files to
	the first argument of the program.

	Synopsis:
	$ charset <charset> <file(s)>

	Arguments:
	<charset>	character-set string to set on the given files
	<file(s)>	files to get a new character set

	Returns:
	==0		for normal operation success
	!=0		some error

*******************************************************************************/

#include	<envstandards.h>	/* ordered first to configure */
#include	<sys/stat.h>		/* POSIX */
#include	<sys/xattr.h>		/* POSIX */
#include	<unistd.h>		/* POSIX */
#include	<fcntl.h>		/* POSIX */
#include	<cstddef>		/* CSTD |nullptr_t| */
#include	<cstdlib>		/* CSTD */
#include	<cstdarg>		/* CSTD */
#include	<cstring>		/* CSTD |strlen(3c)| yes-we-want-it */
#include	<iostream>		/* C++STD */
#include	<clanguage.h>		/* LIBU */
#include	<usysbase.h>		/* LIBU */
#include	<usyscalls.h>		/* LIBU */
#include	<usysutility.hh>	/* LIBU |snvprintf(3u)| */
#include	<readln.hh>		/* LIBU */
#include	<strnul.hh>		/* LIBU */
#include	<mapex.h>		/* LIBU */
#include	<localmisc.h>		/* LIBU */
#include	<deb.hh>		/* |DEBPRINT(3uc)| */

#pragma		GCC dependency		"mod/libutil.ccm"
#pragma		GCC dependency		"mod/ureserve.ccm"
#pragma		GCC dependency		"mod/ulibvals.ccm"
#pragma		GCC dependency		"mod/fonce.ccm"
#pragma		GCC dependency		"mod/deb.ccm"

import libutil ;			/* |getlenstr(3u)| */
import ureserve ;
import ulibvals ;			/* |maxpathlen(3u)| */
import fonce ;
import deb ;				/* |debprintf(3u)| */

/* local defines */

#define	NENTS		1000

#ifndef	CF_DEBUG
#define	CF_DEBUG	0		/* debugging */
#endif


/* imported namespaces */

using std::istream ;			/* type */
using libu::snvprintf ;			/* subroutine */
using libu::rmchr ;			/* subroutine */
using libu::rmtrailchr ;		/* subroutine */
using libu::matstr;			/* subroutine */
using libu::strnchr ;			/* subroutine */
using std::cin ;			/* variable */
using std::cerr ;			/* variable */


/* local typedefs */


/* external subroutines */


/* external variables */


/* local structures */

namespace {
    enum proginfomems {
	proginfomem_start,
	proginfomem_finish,
	proginfomem_flistbegin,
	proginfomem_flistend,
	proginfomem_overlast
    } ; /* end enum */
    struct proginfo ;
    struct proginfo_co {
	proginfo	*op = nullptr ;
	int		w = -1 ;
	void operator () (proginfo *p,int m) noex {
	    op = p ;
	    w = m ;
	} ;
	int operator () (int = 1) noex ;
	operator int () noex {
	    return operator () (1) ;
	} ;
    } ; /* end struct (proginfo_co) */
    struct proginfo {
	friend		proginfo_co ;
	proginfo_co	start ;
	proginfo_co	finish ;
	proginfo_co	flistbegin ;
	proginfo_co	flistend ;
	fonce		seen ;
	mainv		argv ;
	mainv		envv ;
	cchar		*pn = nullptr ;
	cchar		*charsetval = nullptr ;
	int		argc ;
	int		pm = 0 ;
	bool		ffound = false ;
	proginfo(int c,mainv a,mainv e) noex : argc(c), argv(a), envv(e) { 
	    start	(this,proginfomem_start) ;
	    finish	(this,proginfomem_finish) ;
	    flistbegin	(this,proginfomem_flistbegin) ;
	    flistend	(this,proginfomem_flistend) ;
	} ; /* end ctor */
	proginfo() noex : proginfo(0,nullptr,nullptr) { } ;
	void operator () (int c,mainv a,mainv e) noex {
	    argc = c ;
	    argv = a ;
	    envv = e ;
	} ; /* end method */
	int charset	() noex ;
	int charprocess	(char *,int) noex ;
	int readin	(char *,int) noex ;
	int fileproc	(char *,int,cchar *,int = -1) noex ;
	int fileproc_cs	(char *,int,cchar *,int = -1) noex ;
	int fileproc_cm	(char *,int,cchar *,int = -1) noex ;
	int filecheck	(char *,int,cchar *,int = -1) noex ;
	int printf	(cchar *,...) noex ;
    private:
	int getpn	(mainv) noex ;
	int istart	() noex ;
	int ifinish	() noex ;
	int iflistbegin	(int) noex ;
	int iflistend	() noex ;
    } ; /* end struct (proginfo) */
} /* end namespace */


/* forward references */


/* local variables */

#define	PROGMODES \
    	X(charset,	"charset"	)	\
    	X(chstaff,	"chstaff"	)	\
	X(overlast,	nullptr		)

enum progmodes {
#define	X(e,n)	progmode_##e,
    	PROGMODES
#undef	X
} ; /* end enum (progmodes) */

constexpr cpcchar	prognames[] = {
#define	X(e,n)	n,
    	PROGMODES
#undef	X
} ; /* end array (prognames) */

constexpr MAPEX		mapexs[] = {
	{ SR_NOENT,	EX_NOUSER	},
	{ SR_AGAIN,	EX_TEMPFAIL	},
	{ SR_DEADLK,	EX_TEMPFAIL	},
	{ SR_NOLCK,	EX_TEMPFAIL	},
	{ SR_TXTBSY,	EX_TEMPFAIL	},
	{ SR_NOSPC,	EX_TEMPFAIL	},
	{ SR_ACCESS,	EX_NOPERM	},
	{ SR_REMOTE,	EX_PROTOCOL	},
	{ SR_INTR,	EX_INTR		},
	{ SR_EXIT,	EX_TERM		},
	{ SR_NOMSG,	EX_OSERR	},
	{ SR_NOSYS,	EX_OSFILE	},
	{ SR_INVALID,	EX_USAGE	},
	{ 0, 0 }
} ; /* end array (mapexs) */

constexpr cchar		xaname[]	= "charset" ;

static cint		maxpathlen	= ulibval.maxpathlen ;
static cint		maxlinelen	= ulibval.maxline ;
static cint		pagesz		= ulibval.pagesz ;

cint			nents		= NENTS ;

cbool			f_debug		= CF_DEBUG ;


/* exported variables */


/* exported subroutines */

int main(int argc,con mainv argv,con mainv envv) {
    	constexpr int	dfd = (f_debug) ? FD_STDERR : -1 ;
	int		ex = EX_OK ;
	int		rs ;
	int		rs1 ;
	debfd(dfd) ;
	if (proginfo pi(argc,argv,envv) ; (rs = pi.start) >= 0) {
            switch (pi.pm) {
            case progmode_charset:
            case progmode_chstaff:
                rs = pi.charset() ;
                break ;
	    default:
		rs = SR_NOSYS ;
		break ;
            } /* end switch */
	    rs1 = pi.finish ;
	    if (rs >= 0) rs = rs1 ;
	} /* end if (proginfo) */
	if ((ex == EX_OK) && (rs < 0)) {
	    cerr << "charset" << ": error (" << rs << ")" << eol ;
	    ex = mapex(mapexs,rs) ;
	}
	return ex ;
}
/* end subroutine (main) */


/* local subroutines */

int proginfo::charset() noex {
	cnullptr	np{} ;
	cnothrow	nt{} ;
	int		rs = SR_INVALID ;
	int		rs1 ;
	int		c = 0 ; /* return-value */
	if ((argc >= 2) && (charsetval = argv[1]) != np) {
	    if ((rs = flistbegin(nents)) >= 0) {
		if ((rs = pagesz) >= 0) {
		    cint	vlen = rs ;
		    if (char *vbuf = new(nt) char[vlen+1] ; vbuf) {
		        {
	                    rs = charprocess(vbuf,vlen) ;
		            c = rs ;
		        }
		        delete [] vbuf ;
	            } /* end if (m-a-f) */
		} /* end if (pagesz) */
	        rs1 = flistend ;
	        if (rs >= 0) rs = rs1 ;
	    } /* end if (flist) */
	} /* end if (had a character-set specification) */
	return (rs >= 0) ? c : rs ;
} /* end method (proginfo::charset) */

int proginfo::charprocess(char *vbuf,int vlen) noex {
	int		rs = SR_OK ;
	int		c = 0 ;
	for (int ai = 2 ; (rs >= SR_OK) && (ai < argc) && argv[ai] ; ai += 1) {
            if (cchar *fn = argv[ai] ; fn[0]) {
                if ((fn[0] == '-') && (fn[1] == '\0')) {
                    rs = readin(vbuf,vlen) ;
                    c += rs ;
                } else {
                    rs = filecheck(vbuf,vlen,fn) ;
                    c += rs ;
                }
            } /* end if (non-empty) */
            if (rs < 0) break ;
	} /* end for */
	return (rs >= 0) ? c : rs ;
} /* end subroutine (proginfo::charprocess) */

int proginfo::readin(char *vbuf,int vlen) noex {
	cnothrow	nt{} ;
	istream		*isp = &cin ;
	int		rs ;
	int		c = 0 ;
	if ((rs = maxpathlen) >= 0) {
	    cint	llen = rs ;
	    rs = SR_NOMEM ;
	    if (char *lbuf = new(nt) char[llen+1] ; lbuf) {
	        while ((rs = readln(isp,lbuf,llen)) > 0) {
		    int		ll = rs ;
		    if ((ll > 0) && (lbuf[ll - 1] == eol)) ll -= 1 ;
	if_constexpr (f_debug) {
	    strnul fn(lbuf,ll) ;
	    DEBPRINTF("fn=>%s< ll=%d\n",ccp(fn),ll) ;
	}
		    if (ll > 0) {
		        rs = filecheck(vbuf,vlen,lbuf,ll) ;
			c += rs ;
		    }
		    if (rs < 0) break ;
	        } /* end if (reading lines) */
	        delete [] lbuf ;
	    } /* end if (m-a-f) */
	} /* end if (maxpathlen) */
	DEBPRINTF("ret rs=%d c=%d\n",rs,c) ;
	return (rs >= 0) ? c : rs ;
} /* end method (proginfo::readin) */

int proginfo::filecheck(char *vbuf,int vlen,cc *fp,int µfl) noex {
    	int		rs = SR_OK ;
	int		c = 0 ; /* return-value */
	if (int fl ; (fl = getlenstr(fp,µfl)) >= 0) {
	    if (strnul fn(fp,fl) ; fn.fok) {
	        if (ustat sb ; (rs = u_lstat(fn,&sb)) >= 0) {
		    if (S_ISREG(sb.st_mode)) {
    	                if ((rs = seen.checkin(&sb)) > 0) {
	                    if ((rs = fileproc(vbuf,vlen,fp,fl)) >= 0) {
		                c += rs ;
			    } else {
			        printf("proc file=%s (%d)\n",ccp(fn),rs) ;
			    }
		        } /* end if (seen-checkin) */
	            } /* end if (regular file) */
	        } else if (isNotPresent(rs)) {
		    printf("stat file=%s (%d)\n",ccp(fn),rs) ;
		    rs = SR_OK ;
	        } /* end if (u_lstat) */
		DEBPRINTF("done fn=%s rs=%d\n",ccp(fn),rs) ;
	    } /* end if (valid) */
	} /* end if (getlenstr) */
	return (rs >= 0) ? c : rs ;
} /* end subroutine (proginfo::filecheck) */

int proginfo::fileproc(char *vbuf,int vlen,cc *sp,int sl) noex {
    	int		rs = SR_OK ;
	int		c = 0 ;
        switch (pm) {
        case progmode_charset:
            rs = fileproc_cs(vbuf,vlen,sp,sl) ;
	    c = rs ;
            break ;
        case progmode_chstaff:
            rs = fileproc_cm(vbuf,vlen,sp,sl) ;
	    c = rs ;
            break ;
	default:
	    rs = SR_NOSYS ;
	    break ;
	} /* end switch */
	return (rs >= 0) ? c : rs ;
} /* end subroutine (proginfo::fileproc) */

int proginfo::fileproc_cs(char *vbuf,int vlen,cc *sp,int sl) noex {
	static csize	xasize = strlen(charsetval) ;
	strnul		fn(sp,sl) ;
	cint		rse = SR_EXISTS ;
	cint		xo = XATTR_CREATE ;
	int		rs ;
	int		c = 0 ; /* return-value */
	cchar		*xaval = charsetval ;
	(void) vbuf ;
	(void) vlen ;
	if ((rs = u_xattrset(fn,xaname,xaval,xasize,0u,xo)) == rse) {
	    rs = SR_OK ;
	    c += 1 ;
	}
	DEBPRINTF("ret fn=>%s< c=%d\n",ccp(fn),c) ;
	return (rs >= 0) ? c : rs ;
} /* end method (proginfo::fileproc_cs) */

int proginfo::fileproc_cm(char *vbuf,int vlen,cc *sp,int sl) noex {
	strnul		fn(sp,sl) ;
    	int		rs ;
	(void) vbuf ;
	(void) vlen ;
	if (ustat sb ; (rs = u_stat(fn,&sb)) >= 0) {
	    bool f = false ;
	    f = f || ((sb.st_mode & 0200) == 0) ;
	    f = f || ((sb.st_mode & 0020) == 0) ;
	    if (f) {
		cmode nm = sb.st_mode | 0200 | 0020 ;
		rs = u_chmod(fn,nm) ;
	    }
	} /* end if (u_stat) */
    	return rs ;
} /* end method (proginfo::fileproc_cm) */

int proginfo::istart() noex {
	int		rs ;
	if ((rs = getpn(prognames)) >= 0) {
	    rs = 0 ;
	} /* end if (proginfo::getpn) */
	return rs ;
} /* end method (proginfo::istart) */

int proginfo::ifinish() noex {
	return SR_OK ;
} /* end method (proginfo::ifinish) */

int proginfo::getpn(mainv names) noex {
	int		rs = SR_FAULT ;
	if (argv) {
	    rs = SR_NOMSG ;
	    if ((argc > 0) && argv[0]) {
		cchar	*arg0 = argv[0] ;
	        cchar	*bp{} ;
	        if (int bl ; (bl = sfbasename(arg0,-1,&bp)) > 0) {
		    if (cint pl = rmchr(bp,bl,'.') ; pl > 0) {
	                if ((pm = matstr(names,bp,pl)) >= 0) {
			    pn = names[pm] ;
		            rs = pm ;
	                }
		    } /* end if (non-zero positive progname) */
		} /* end if (have base-name) */
	    } /* end if (have first argument) */
	} /* end if (non-null) */
	return rs ;
} /* end method (proginfo::getpn) */

int proginfo::iflistbegin(int n) noex {
        return seen.start(n) ;
} /* end method (proginfo::iflistbegin) */

int proginfo::iflistend() noex {
	return seen.finish ;
} /* end method (proginfo::iflistend) */

int proginfo::printf(cchar *fmt,...) noex {
	cnothrow	nt{} ;
    	va_list		ap ;
    	int		rs = SR_FAULT ;
	if (fmt) {
	    rs = SR_INVALID ;
	    if (fmt[0]) {
	        va_begin(ap,fmt) ;
	        if ((rs = maxlinelen) >= 0) {
		    cint	llen = rs ;
		    rs = SR_NOMEM ;
		    if (char *lbuf = new(nt) char[llen + 1] ; lbuf) {
	    	        if ((rs = snvprintf(lbuf,llen,fmt,ap)) >= 0) {
			    if (pn == nullptr) pn = "charset" ;
			    cerr << pn << ": " << lbuf ;
		        } /* end if (snvprintf) */
		        delete [] lbuf ;
		    } /* end if (new-char) */
	        } /* end if (maxlinelen) */
	        va_end(ap) ;
	    } /* end if (valid) */
	} /* end if (non-null) */
    	return rs ;
} /* end method (proginfo::printf) */

int proginfo_co::operator () (int a) noex {
	int		rs = SR_BUGCHECK ;
	if (op) ylikely {
	    switch (w) {
	    case proginfomem_start:
	        rs = op->istart() ;
	        break ;
	    case proginfomem_finish:
	        rs = op->ifinish() ;
	        break ;
	    case proginfomem_flistbegin:
	        rs = op->iflistbegin(a) ;
	        break ;
	    case proginfomem_flistend:
	        rs = op->iflistend() ;
	        break ;
	    } /* end switch */
	} /* end if (non-null) */
	return rs ;
} /* end method (proginfo_co::operator) */


