...
int proginfo::tmpmounts_oner(const confstritem *itp) noex {
	...
	int		c = 0 ;
	if (itp->req >= 0) {
	    if ((rs = usysconfstr(pbuf,plen,itp->req)) > 0) {
	        if (cint rl = rmtrailchr(pbuf,rs,'/') ; rl > 1) {
	            pbuf[rl] = '\0' ;
	            if ((rs = u_stat(pbuf,&sb)) >= 0) {
...
int proginfo::tmpmounts_cklink() noex {
	...
	if ((rs = maxpathlen) >= 0) {
		...
		if ((rs = u_readlink(dbuf,lbuf,llen)) > 0) {
		    if (cint ll = rmtrailchr(lbuf,rs,'/') ; ll > 0) {
		        lbuf[ll] = '\0' ;
...
int proginfo::tmpuserdir_link() noex {
	...
	if ((rs = usysconfstr(dbuf,dlen,req)) > 0) {
	    if (cint rl = rmtrailchr(dbuf,rs,'/') ; rl > 1) {
	        dbuf[rl] = '\0' ;
	        if ((rs = u_stat(dbuf,&sb)) >= 0) {
		    if (S_ISDIR(sb.st_mode)) {
