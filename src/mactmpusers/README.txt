...
and to create symbolic links under there to three specialized
...
itself).  The three links created in that directory are:
	...
	tmp	- used for per-user only temporary files
	cache	- used for storing per-user semipersistent cache files
...
Mount (or symbolicly-link) the three temporary-directories to the 
various mount points they are supposed to appear at.
