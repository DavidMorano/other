	...
	if (argv) {
	    ...
	    if ((argc > 0) && argv[0]) {
		    ...
		    if (cint pl = rmchr(bp,bl,'.') ; pl > 0) {
	                if (int i ; (i = matstr(names,bp,pl)) >= 0) {
		            rs = i ;
	                }
...
static int printuserents() noex {
	...
	int		rs = SR_OK ;
	try {
	    for (PASSWD *pwp ; (pwp = getpwent()) != nullptr ; ) {
...
static int printgroupents() noex {
	...
	int		rs = SR_OK ;
	try {
	    for (GROUP *grp ; (grp = getgrent()) != nullptr ; ) {
...
int userinfo::findenv(uid_t uid) noex {
	...
	for (cauto &vn : envs) {
		...
		if (int cl ; (cl = sfbasename(rp,-1,&cp)) > 0) {
		    if (PASSWD *pwp ; (pwp = getpwnam(rp)) != nullptr) {
		        if (pwp->pw_uid == uid) {
	    		    len = load(pwp) ;
		        } /* end if (match) */
		    } /* end if (getpwnam) */
...
int userinfo::findutmp_sid(uid_t uid) noex {
	...
	for (UTMPX *up ; (up = getutxent()) != nullptr ; ) {
		...
		if ((rs = xstrnlen(unp,szof(up->ut_user))) > 0) {
	            char	ubuf[rs+1] ;
	            strwcpy(ubuf,unp,rs) ;
	            if (PASSWD *pwp ; (pwp = getpwnam(ubuf)) != nullptr) {
			if (pwp->pw_uid == uid) {
	    		    len = load(pwp) ;
			} /* end if (uid match) */
	            } /* end if (getpwnam) */
